...
W = {Delay = 0.25}
E = {Delay = 0.25}
R = {Delay = 0.25, Range = 187}
end

function Jax:LoadMenu()
    self.Menu = MenuElement({type = MENU, id = "Jax", name = "Jax - The Prodigal Explorer", leftIcon="http://pt.seaicons.com/wp-content/uploads/2015/07/Jax-Pulsefire-icon.png"})
...
if etarget and self.Menu.Combo.ComboE:Value() and self:CanCast(_E) and self:CanCast(_Q) then
...
local qMinion
        ...
        if minion.isTargetable abd minion.distance <= E.Range and minion.team ~= myHero.team then
            qMinion = minion
            break
        end
			if qMinion then
